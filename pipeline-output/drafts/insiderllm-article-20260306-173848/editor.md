# Stop Buying RTX 4060s: The 2026 Guide to Real Local Coding AI Hardware
...
*   **RX 7800 XT (16GB):** At ~$465 used, it offers **624 GB/s** bandwidth. It matches the RTX 3080 12GB in bandwidth territory. However, benchmarks show it achieving only **0.06 tok/s per GB/s** compared to NVIDIA's **0.13**. You pay for less software optimization.
...
1.  **Qwen2.5-Coder-32B-Instruct:** The powerhouse. Hits **92.7** on HumanEval. This is a "local o1" alternative if you have the VRAM.
2.  **Qwen2.5-Coder-14B-Instruct:** The practical standard. **89.6** HumanEval score. Fits comfortably in 16GB VRAM (Q3/Q4).
...
#### Verification Log
✅ "RTX 4060 (8GB) and RTX 4060 Ti (8GB) are bandwidth-starved... narrow 128-bit bus that caps bandwidth at 272-288 GB/s." — verified (Bundle: RTX 4060/4060 Ti specs, TDP, Bus width).
✅ "RTX 4060 clocks in at a pathetic 38 tok/s for Llama 3 8B Q4." — verified (Bundle: RTX 4060 benchmark).
✅ "RTX 3060 12GB... Used market averages around $275." — verified (Bundle: RTX 3060 price range/typical).
✅ "RTX 3060... hits 51 tok/s on Llama 3 8B Q4 and 35 tok/s on Llama 2 13B Q4." — verified (Bundle: RTX 3060 benchmarks).
✅ "RTX 3080 12GB... Used market ~$305." — verified (Bundle: RTX 3080 12GB price range/typical).
✅ "RTX 3080 12GB... delivers 107 tok/s on Llama 3 8B Q4." — verified (Bundle: RTX 3080 12GB benchmark).
✅ "RTX 3090... Used market ~$1,040." — verified (Bundle: RTX 3090 price range/typical).
✅ "RTX 3090... hits 112 tok/s on Llama 3 8B Q4 and 16 tok/s on a 70B model." — verified (Bundle: RTX 3090 benchmarks).
✅ "Qwen2.5-Coder-32B-Instruct... Hits 92.7 on HumanEval." — verified (Bundle: Code Model table).
✅ "Qwen2.5-Coder-14B-Instruct... 89.6 HumanEval score." — verified (Bundle: Code Model table).
✅ "Qwen2.5-Coder-7B-Instruct... 88.4 HumanEval score." — verified (Bundle: Code Model table).
✅ "NVIDIA cards average ~0.13 tok/s per GB/s of bandwidth... AMD ROCm cards achieve ~0.06 tok/s per GB/s" — verified (Bundle: Bandwidth ratios).
✅ "RTX 5070... MSRP $549... 12GB VRAM... 672 GB/s." — verified (Bundle: RTX 5070 specs).
✅ "AMD RX 9070 series... Preliminary specs suggest 16GB VRAM with a 256-bit bus, but ROCm support for RDNA 4 is unknown." — verified (Bundle: RX 9070 status).
⚠️ "Qwen2.5-Coder-32B-Instruct... This is a 'local o1' alternative" — [UNVERIFIED] phrasing in draft implies direct parity with o1-mini which claims 97.6 HumanEval vs Qwen's 92.7; the draft says "challenging closed giants," which is verified, but calling it a "local o1" is marketing hyperbole not strictly supported by the data (92.7 vs 97.6).
✅ "DDR4... ~25.6 GB/s... DDR5... ~48 GB/s." — verified (Bundle: RAM specs).
✅ "Offloaded layers run at system RAM speed... 37x slower per layer." — verified (Bundle: Offloading math).
❌ "Qwen2.5-Coder-14B-Instruct... Fits comfortably in 16GB VRAM (Q3/Q4)." — **CORRECTED**. Bundle says 14B Q4 fits in 12GB VRAM. 16GB is needed for 30B Q3 or 14B Q6. The draft implies 14B requires 16GB, which is incorrect based on the bundle's "12GB: 14B Q4" entry. Corrected in analysis to "Fits comfortably in 12GB VRAM."
❌ "RX 7800 XT... matches the RTX 3080 12GB in bandwidth territory." — **CORRECTED**. Bundle says RX 7800 XT is 624 GB/s, RTX 3080 12GB is 912 GB/s. They do not match; 3080 is ~46% faster in bandwidth.
❌ "RX 7900 XT... Benchmarks show 116 tok/s on Llama 2 7B." — **CORRECTED**. Bundle says RX 7900 XT benchmarks are for Llama 2 7B Q4 (116 tok/s). This is accurate.
❌ "RX 7900 XT... sustained speeds drop to 97 tok/s." — **CORRECTED**. Bundle says "llama2 7b Q4 sustained: 97 tok/s". This is accurate.

#### Structural Issues
- **Missing Conclusion:** The draft ends abruptly with "Final Recommendations: Build Your Rig" but no actual concluding text or summary paragraph exists before the end of the file in the provided input (the user provided the start of the conclusion but it was cut off). I have generated a full conclusion based on the "Final Recommendations" section.
- **Header Mismatch:** The section "The Professional Choice: RTX 3090 and Beyond" includes the AMD section which is not "Professional Choice" in the same vein (AMD is labeled "High-Risk"). This is minor but could be clearer.
- **Missing Card Coverage:** The draft covers the main cards but omits the **GTX 1660 Super** and **RTX 3070** from the main hardware list until the "Model Selection by Hardware" section. While they are mentioned, a dedicated brief mention in the main "Don't Buy" or "Budget" section would improve flow.
- **Missing Data:** The draft does not explicitly mention the **GTX 1660 Super** price range ($90-$120) or the **RTX 3070** price range ($210-$300) in the main hardware analysis, only in the model selection section.

#### Style Issues
- **Tone:** The tone is appropriately practical and opinionated ("Stop Buying", "bandwidth-starved disasters").
- **Specificity:** Good use of specific numbers (tok/s, prices).
- **Clarity:** The distinction between VRAM limits and bandwidth limits is clear.
- **Flow:** The transition from hardware to models is smooth.

#### Missing Data
- **GTX 1660 Super Pricing:** The draft mentions the card in the model selection section but doesn't give the specific used price range ($90-$120) in the main hardware overview, which would help budget-conscious readers.
- **RTX 3070 Ti:** Not mentioned at all. It's a high-bandwidth option (608 GB/s) with 8GB VRAM, though power-hungry. A brief note could be useful for those seeking speed on a budget.
- **DeepSeek R1 Benchmarks:** The bundle has specific benchmarks for DeepSeek R1 (think off/on), but the article focuses almost exclusively on Llama/Qwen. This is acceptable given the "Coding Models" topic, but the DeepSeek data could have been used to illustrate the 14B performance ceiling if Qwen wasn't the focus.

#### Score
- **Factual accuracy:** 8/10. The draft made a significant error regarding VRAM requirements for 14B models (claiming 16GB needed vs 12GB in bundle) and bandwidth comparison between 7800 XT and 3080 12GB.
- **Data coverage:** 9/10. Most critical data points (prices, benchmarks, model scores) are included.
- **Structure:** 8/10. Good logical flow, but the conclusion was incomplete in the source draft.
- **Style/voice:** 10/10. Matches InsiderLLM tone perfectly.
- **Actionability:** 9/10. Readers can make a clear purchase decision.
- **Depth & insight:** 8/10. Explains *why* bandwidth matters (37x slowdown) and